...
Esta otimização foi utilizada na implementação dos testes de desempenho, conforme o \tratar{Algoritmo \ref{alg:matriz} (refazer com entrada e saída enunciadas}.
...
Usando apenas as operações que podem ser vistas \tratar{nos algoritmos [X] (ancestral comum novo) e [Y] (função de espalhamento) mais adiante},
...
\tratar{Algoritmo ancestral comum novo}
...
\tratar{O Algoritmo X apresenta como este método funciona. Explicar}

\tratar{[Algoritmo da função sem colisões]}
