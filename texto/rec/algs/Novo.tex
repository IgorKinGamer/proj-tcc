\begin{algorithm}
	...
	\Entrada{Dois nós $a$ e $b$ \newline
	         ...
	         Uma função \bpn, que retorna o nível
	          ...
	          dado um número com apenas um \textit{bit} 1,
	...
	%      é a posição do primeiro \textit{bit} 1 em um número,
	...
	\tcp{\textit{Bits} que diferem}
	$\var{dif} \gets \id{a} \OuExBit \id{b}$ \\
	\tcp{\textit{Bit} 1 apenas na primeira posição em que os IDs diferem}
	$\var{bit} \gets \var{dif} \EBit (-\var{dif})$ \\
	\tcp{Todos os \textit{bits} antes do primeiro diferente}
...
\begin{algorithm}
	...
	\Nota {Os valores $\var{ad}_{n}$, $\var{m}_{n}$,
	       ...
	       $x \Desl b$ é o valor $x$ deslocado $b$ \textit{bits} para a direita,
