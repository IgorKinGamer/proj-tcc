\documentclass[a5paper]{ufsc-thesis}

% UTF-8
\usepackage[utf8]{inputenc}
% Fonte
\usepackage[T1]{fontenc}
\usepackage{lmodern}
% Português
\usepackage[brazil]{babel}


%%% BIBLIOGRAFIA %%%
% Indicar páginas com as citações na bibliografia
%\usepackage[brazilian,hyperpageref]{backref}
% Citações padrão ABNT
\usepackage[alf]{abntex2cite}


%%% OUTROS PACOTES %%%
% Matemática
\usepackage{amsmath}
\usepackage{amssymb}
% TikZ
\usepackage{tikz}
\usetikzlibrary{positioning}
% Código fonte
\usepackage{listings}
% Novos comands
\usepackage{xparse}
% Sub-figuras
\usepackage{subcaption}
% Frescura (®, TM)
\usepackage{textcomp}
% Multilinha
\usepackage{multirow}
% Hifenação
\usepackage{hyphenat}
\hyphenation{HieSchella}
%
\usepackage{microtype}
\emergencystretch 3em

%%% NOVOS COMANDOS %%%
% Destaque - coisas a tratar
\newcommand \tratar[1] {\textcolor{red}{!!! #1 !!!}}
% Notação O
\DeclareMathOperator \bigO {O}
\newcommand \Oalt {$\bigO(\mathit{altura})$}
% ...
\newcommand \no[1] {$#1$}
\DeclareMathOperator \pai {pai}
\newcommand \id[1]    {\ensuremath{\mathrm{id}_{#1}}}
\newcommand \idstr[1] {\ensuremath{\id{#1}^{\mathrm{str}}}}
\newcommand \fACMP {$\mathrm{ACMP}$}
\newcommand \ACMPIDs {$\mathrm{ACMP}_{\mathrm{IDs}}$}
% Nome dos algoritmos
\newcommand \nomeSimples {AscendACMP}
\newcommand \nomeNovo    {CombinACMP}
\newcommand \nomeHwloc   {OndulACMP}
\newcommand \nomeMatriz  {MatrizACMP}
\newcommand \Simples {\mbox{\textsc{\nomeSimples}}}
\newcommand \Novo    {\mbox{\textsc{\nomeNovo}}}
\newcommand \Hwloc   {\mbox{\textsc{\nomeHwloc}}}
\newcommand \Matriz  {\mbox{\textsc{\nomeMatriz}}}
\newcommand \Espalha {\mbox{\textsc{Espalhamento}}}
%\hyphenation{\nomeSimples}
%\hyphenation{\nomeNovo}
%\hyphenation{\nomeHwloc}
%\hyphenation{\nomeMatriz}
%\hyphenation{Espalhamento}
% Funções
\newcommand \bpn {\mbox{\textsc{BitParaNível}}}


%%% ALGORITMOS %%%
\usepackage[portuguese, ruled, vlined]{algorithm2e}
%\usepackage{algpseudocode}
%\usepackage{algorithm}
%\floatname{algorithm}{Algoritmo}
\newcommand \var[1] {\mathrm{#1}}
\newcommand \Cmnt[1] {\textcolor{green!50!black}{// #1}} %alg
\DeclareMathOperator \OuExBit {\ \mathbf{OuEx_{bit-a-bit}}\ }
\DeclareMathOperator \EBit    {\ \mathbf{E_{bit-a-bit}}\ }
\DeclareMathOperator \Desl    {\ \mathbf{Desl_{direita}}\ }
\newcommand \fonteCmnt[1] {\ttfamily\textcolor{gray}{#1}}
\SetCommentSty{fonteCmnt}


%\usepackage{cmap} ???


\usepackage[printonlyused]{acronym} 


\input{rec/Especificacoes}


\begin{document} %%%

%%%%%%%%%%%%%%%%%
%%% PRÉ-TEXTO %%%
%%%%%%%%%%%%%%%%%
\pretextual

% Inicia parte pré-textual do documento capa, folha de rosto, folha de
% aprovação, aprovação, resumo, lista de tabelas, lista de figuras, etc.
\include{rec/Pre}

%\include{init/pretextuais}
%\clearpage
%\imprimirfichacatalografica
%\tableofcontents


%%%%%%%%%%%%%%%%%
%%%   TEXTO   %%%
%%%%%%%%%%%%%%%%%
\textual

\include{rec/caps/Introducao}
\include{rec/caps/Fundamentacao_teorica}
\include{rec/caps/Estado_da_arte}
\include{rec/caps/Ancestral}
\include{rec/caps/Testes}
\include{rec/caps/Conclusao}


%%%%%%%%%%%%%%%%%
%%% PÓS-TEXTO %%%
%%%%%%%%%%%%%%%%%
\postextual

\bibliography{rec/ref}

\include{rec/Apendice}

\end{document}